...
public class MeshDestroyer
{
    ...
    public List<GameObject> DestroyMesh(GameObject objectToDestroy, int numberOfParts)
    {
        this.objectToDestroy = objectToDestroy;
        this.CutCascades = numberOfParts;

        var originalMesh = this.objectToDestroy.GetComponent<MeshFilter>().mesh;
        ...
        List<GameObject> partGameObjects = new List<GameObject>();
       

        for (var i = 0; i < parts.Count; i++)
        {
            partGameObjects.Add(parts[i].MakeGameobject(this));
        }
    ...
    public class PartMesh
    {
        ...
        public GameObject MakeGameobject(MeshDestroyer original)
        {
            GameObject = new GameObject(original.objectToDestroy.name);

            var mesh = new Mesh();
#if UNITY_EDITOR
            mesh.name = original.objectToDestroy.GetComponent<MeshFilter>().sharedMesh.name;
#else
            mesh.name = original.objectToDestroy.GetComponent<MeshFilter>().mesh.name;
#endif
...
#if UNITY_EDITOR
            renderer.sharedMaterial = original.objectToDestroy.GetComponent<MeshRenderer>().sharedMaterial;
            filter.sharedMesh = mesh;
#else
            renderer.materials = original.objectToDestroy.GetComponent<MeshRenderer>().materials;
            filter.mesh = mesh;
#endif
